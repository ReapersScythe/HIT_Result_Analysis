...
worker_result = [['worker_id','# of outliers', 'Questions Done', 'consistent percentage']] # Header for worker analysis report
...
for i in workers:
    worker_tasks_number[list(i.keys())[0]] = len(list(i.values())[0].keys())
...
for i in workers:
    ...
    for k in i[list(i.keys())[0]].values():
        if k == 'outlier':
            t += 1
    percentage = t * 1.0 / worker_tasks_number[list(i.keys())[0]]
    worker_result.append([list(i.keys())[0], t, worker_tasks_number[list(i.keys())[0]], 1 - percentage])
    print('Worker_Id: ' + list(i.keys())[0] + ' | # of outliers = ' + str(t) + ' | Total Questions Done: ' + str(worker_tasks_number[list(i.keys())[0]]) + ' | consistent percentage: ' + str(1 - percentage) )
